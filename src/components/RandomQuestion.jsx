...
export default function RandomQuestion() {
  ...
  const formattedTime = (timeReceveid) => {
    return `${Math.floor(timeReceveid / 60)
      .toString()
      .padStart(1, '0')}:${(timeReceveid % 60).toString().padStart(2, '0')}`;
  };
    ...
    <div className="flex flex-col w-full min-h-screen m-auto justify-center items-center">
      ...
      <div className="flex flex-col md:flex-row w-full items-center gap-10 justify-center">
        <div className="flex flex-col justify-center items-center w-full sm:w-[85%] md:w-[100%]">
          <h1 className="text-white text-center text-2xl font-semibold min-h-[150px] md:min-h-[180px] lg:min-h-[150px] mx-10 sm:text-3xl">
            {gotQuestion ? question : 'Your question will appear here 😁'}
          </h1>
          <button
            onClick={() => handleSelectQuestion()}
            className="w-[60%] lg:w-[40%] text-white cursor-pointer border-2 rounded-2xl p-1"
          >
        ...
        <div className="flex flex-col justify-center items-center sm:mt-2 md:mx-10 w-1/2">
          <h1 className="text-white text-7xl sm:text-8xl min-h-[100px] md:min-h-[180px] lg:min-h-[150px] md:text-8xl">
            {stopwatch ? formattedTime(time) : formattedTime(0)}
          </h1>
          <div className="flex flex-row gap-2">
            <button
              onClick={() => handleStopWatch()}
              className="text-white cursor-pointer border-2 rounded-2xl p-1"
            >
              {stopwatch ? (isPaused ? 'Resume' : 'Pause') : 'Start Stopwatch'}
            </button>
            <button
              onClick={() => handleReset()}
              className="text-white cursor-pointer border-2 rounded-2xl p-1"
